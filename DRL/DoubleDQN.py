...
class DoubleDQN(DRL):
    def __init__(self, sess,  scope, globalAC=None):
        ...
        self.lr = cfg['DoubleDQN']['LR']
        self.gamma = cfg['DoubleDQN']['GAMMA']
        self.e_greedy = cfg['DoubleDQN']['E_GREEDY']
        self.update_nn_step = cfg['DoubleDQN']['UPDATE_NN_STEP']
        self.memory_size = cfg['DoubleDQN']['MENORY_SIZE']
        self.batch_size = cfg['DoubleDQN']['BATCH_SIZE']

        if scope != cfg['DoubleDQN']['main_net_scope']:
    ...
    def train(self,  states, actions, rewards, next_state, done):
        ...
        if (not done) and (self.memory_counter > 1000):
            ...
            q_eval = self.sess.run(self.q_eval, {self.s: batch_memory[:, :cfg['RL']['state_shape'][0]]})
